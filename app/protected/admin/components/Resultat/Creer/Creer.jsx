...
const Creer = () => {
  ...
  const handleCreateResult = async () => {
    ...
    try {
      const res = await fetch("/api/crud/admin/create/resultat", {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setMessageError("Une erreur s'est produite l'or de la creation du Resultat !")
      }
    } catch (error) {
      setNotificationValid(false)
      setNotificationError(true)
  ...
  useEffect(() => {
    const fetchinEtudes = async () => {
      try {
        const res = await fetch("/api/crud/admin/get-all/etudes/resultat", {
          method: "GET"
        })
        if (res.ok) {
          const data = await res.json()
          const etudes = data.etudesSansResultat

          SetEtudes(etudes.map(etude => {
            return etude
          }))
  ...
  return (
    <div className={`${styles.creer} ${styles.backgroungCreer}`}>
      ...
      <Autocomplete
        renderOption={(props, option) => (
          <li {...props} key={option.id}>
            {option.nom_etude ? option.nom_etude : "Sans nom"}
          </li>
        )}
      ...
      <div className={styles.btnSubmit}>
        <Button onClick={handleCreateResult} sx={{ color: "white" }} >Creer le Resultat</Button>
      </div>
