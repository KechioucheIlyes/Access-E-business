...
export default function TableCollapsibleRow() {
    ...
    const handleContrat = async (id) => {
        if (!id) {
            setNoContrat(true)
        } else {
            try {
                const res = await fetch(`/api/crud/admin/contratpdf/${id}`, {
                    method: "GET",
    ...
    const displayPdf = (pdfArray) => {
        const blobUrl = URL.createObjectURL(pdfArray);
        window.open(blobUrl);
    };
                ...
                if (response.ok) {
                    ...
                    if (data) {
                        ...
                        const newData2 = commission.commissions.map(com => {
                            ...
                            return createData(
                                `${com.Etudes[0] ? com.Etudes[0].nom_etude : null}`,
                                `${com.Etudes[0] ? com.Etudes[0].Clients.nom_entreprise : "/"}`,
                                `${com.Etudes[0] ? com.Etudes[0].statut : null}`,
                                `${new Date(com.Etudes[0] ? com.Etudes[0].date_debut : null).toLocaleDateString("fr-FR")}`,
                                <Image className={com.Etudes[0] ? com.Etudes[0].contrats[0].fichier_contrat ? styles.pdf : styles.nopdf : styles.nopdf} onClick={() => { handleContrat(com.Etudes[0] ? com.Etudes[0].contrats[0].id : null) }} src={com.Etudes[0] ? com.Etudes[0].contrats[0].fichier_contrat ? pdf : pdfGrey : pdfGrey} alt='pdf' height={30} width={30} />,
                                "1.99",
                                historyData2
                            )
                        })
                ...
                >
                    <thead style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>
                        <tr style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>
                            ...
                            <th style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white", width: '20%' }}>Études</th>
                            <th style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>Entreprise</th>
                            <th style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>Statut</th>
                            <th style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>date de signature</th>
                            <th style={{ backgroundColor: 'transparent', color: "white", borderBottom: "2px solid white" }}>Contrats</th>
