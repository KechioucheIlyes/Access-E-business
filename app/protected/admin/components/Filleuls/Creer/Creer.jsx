...
const Creer = () => {
    ...
    const handleSelectedPercent = (_, percent) => {
        if (percent) {
            setPourcentages(percent.label)
        } else {
            setPourcentages('')
        }
    }
    ...
    const handleCreateFilleul = async () => {

        if (!raisonSociale || !secteurActivite || !nom || !prenom || !fonction || !email || !fixe || !mobile || !pourcentages || !userID) {
            ...
            try {
                const res = await fetch("/api/crud/admin/create/filleul", {
                    method: "POST",
                    headers: {
                ...
                if (res.ok) {
                    ...
                    setNotificationValid(false)
                    setNotificationError(true);
                    setMessageError("Erreur l'or de la création du filleul veuillez reessayer plus tard !")
                }
            } catch (error) {
                setNotificationValid(false)
                setNotificationError(true);
    ...
    return (
        <div className={`${styles.creer} ${styles.backgroungCreer}`} >
            ...
            <Autocomplete
                ...
                id="combo-box-demo"
                options={pourcentage}
                onChange={handleSelectedPercent}
                sx={{
                    color: "#716c6c",
                    width: 300,
                    "& .MuiOutlinedInput-root": {
            ...
            <div className={styles.btnSubmit}>
                <Button sx={{ color: "white" }} onClick={handleCreateFilleul} >Créer le filleul </Button>
            </div>
        </div>
    )
}
